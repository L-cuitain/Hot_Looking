<template>
  ...
  <!--  分页-->
  <Pagination :listInfo="articlesInfo" @getInfo="getData" />
</template>
...
export default {
  ...
  setup() {
    const { articlesInfo, getData } = useArticlesList();

    return { articlesInfo, getData };
  },
};

function useArticlesList() {
  const articlesInfo = ref();

  const getData = (current) => {
    getArticlesAll(current).then((data) => {
      articlesInfo.value = data.result;
    });
  };
  return { articlesInfo, getData };
}
...
.articles_info_img_cover p {
  display: none;
  font-size: 0.875rem;
}

.articles_info_img_cover:hover {
  background: rgba(0, 0, 0, 0.4);
  color: #f7f3f0;
}

.articles_info_img_cover:hover p {
  display: block;
}
...
.articles_info_category {
  font-size: 0.75rem;
  line-height: 1.1;
  color: #ff3d1d;
}

.articles_info_content {
...
.articles_info_btm_user {
  display: inline-flex;
  font-size: 10px;
  line-height: 16px;
}

.articles_info_btm_user > img {
  width: 26px;
  height: 26px;
  margin: 4px 0;
  border-radius: 50%;
...
.articles_info_btm_comment > span {
  vertical-align: -3%;
}
