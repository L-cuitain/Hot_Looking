  ...
  <div class="nav">
    <div class="container">
      <el-row :gutter="24">
        <el-col :span="16">
          <!--      轮播图-->
          <Carousel class="p_2" :data="recommendVideo" />
        </el-col>
        <el-col :span="8">
          <!--        推荐专题  -->
...
export default {
  ...
  setup() {
    const { recommendCollection } = useGetRTopics();
    const { recommendVideo } = useGetHotVideo();
    const { recommendNews } = useGetRNews();
    return { recommendCollection, recommendVideo, recommendNews };
  },
};

//获取首页订阅数最多的专题
function useGetRTopics() {
  const recommendCollection = ref();
  const getData = () => {
    getRecommendCollection().then((data) => {
      recommendCollection.value = data.result;
    });
  };
...
//获取首页推荐视频
function useGetHotVideo() {
  const recommendVideo = ref();

  const getData = () => {
    getHotVideo().then((data) => {
      recommendVideo.value = data.result;
    });
  };
...
//获取首页推荐资讯
function useGetRNews() {
  const recommendNews = ref();

  const getData = () => {
    getRecommendNews().then((data) => {
      recommendNews.value = data.result;
    });
  };
