<template>
  <div class="list_row" v-if="topInfo">
    ...
    <!--        分页-->
    <Pagination :listInfo="topInfo" @getInfo="getData" />
  </div>
</template>

<script>
import { ref } from "vue";
import { getAVAll } from "../../../api/top";
...
export default {
  ...
  setup() {
    const { topInfo, getData } = useTopList();
    getData(1);
    return { topInfo, getData };
  },
};
...
function useTopList() {
  ...
  const getData = (current) => {
    getAVAll(current).then((data) => {
      topInfo.value = data.result;
    });
  };
  return { topInfo, getData };
}
