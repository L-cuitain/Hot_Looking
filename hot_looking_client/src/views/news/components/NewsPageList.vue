<template>
  ...
  <!--  分页-->
  <Pagination :listInfo="newsInfo" @getInfo="getData" />
</template>
...
export default {
  ...
  setup() {
    const { newsInfo, getData } = useNewsList();

    return { newsInfo, getData };
  },
};
...
function useNewsList() {
  ...
  const getData = (current) => {
    getNewsList(current).then((data) => {
      newsInfo.value = data.result;
    });
  };
