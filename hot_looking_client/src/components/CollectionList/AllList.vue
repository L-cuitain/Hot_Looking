...
import { getAllList } from "@/api/collections";
...
export default {
  ...
  setup() {
    ...
    const { allListInfo, getData } = useAllList(lId);
    getData(1);

    return { allListInfo, getData };
  },
};

function useAllList(lId) {
  const allListInfo = ref();

  const getData = (current) => {
    getAllList(lId, current).then((data) => {
