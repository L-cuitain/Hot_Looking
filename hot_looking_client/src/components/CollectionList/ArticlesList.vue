<template>
  <div class="list_row" v-if="articlesListInfo">
    <el-empty
      description="还没有内容"
      v-if="articlesListInfo.total === 0"
    ></el-empty>
    <div v-else>
      <el-row :gutter="24">
        <el-col
      ...
      <!--  分页-->
      <Pagination :listInfo="articlesListInfo" @getInfo="getData" />
    </div>
  </div>
</template>
...
import { getArticlesList } from "@/api/collections";
...
export default {
  ...
  setup() {
    ...
    const { articlesListInfo, getData } = useArticlesList(lId);
    getData(1);

    return { articlesListInfo, getData };
  },
};

function useArticlesList(lId) {
  const articlesListInfo = ref();

  const getData = (current) => {
    getArticlesList(lId, current).then((data) => {
      articlesListInfo.value = data.result;
    });
  };
  return { articlesListInfo, getData };
}
