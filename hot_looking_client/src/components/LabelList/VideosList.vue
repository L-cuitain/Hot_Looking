<template>
  <div class="list_row" v-if="videosListInfo">
    ...
    <!--  分页-->
    <Pagination :listInfo="videosListInfo" @getInfo="getData" />
  </div>
...
import { getVideosList } from "../../api/label";
...
export default {
  ...
  setup() {
    ...
    const { videosListInfo, getData } = useVideosList(lId);
    getData(1);

    return { videosListInfo, getData };
  },
};

function useVideosList(lId) {
  const videosListInfo = ref();

  const getData = (current) => {
    getVideosList(lId, current).then((data) => {
      videosListInfo.value = data.result;
    });
  };
  return { videosListInfo, getData };
}
